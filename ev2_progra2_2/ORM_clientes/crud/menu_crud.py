...
class MenuCRUD:
    ...
    @staticmethod
    def crear_menu(db: Session, nombre: str, descripcion: str, lista_ingredientes: list, precio: float):
        ...
        items_validos = list(
            filter(lambda item: item[1] > 0, lista_ingredientes))
        ...
        objs_ingredientes = db.query(Ingrediente).filter(
            Ingrediente.id.in_(ids_unicos)).all()
        dict_ingredientes = {ing.id: ing for ing in objs_ingredientes}

        def verificar_stock(item): return (
            item[0] in dict_ingredientes and
            dict_ingredientes[item[0]].cantidad >= item[1]
        )

        es_posible_crear = reduce(
            lambda acumulado, item: acumulado and verificar_stock(item), items_unicos, True)
